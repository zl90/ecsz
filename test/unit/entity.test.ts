...
import { World, Component, Types } from "../../src/index";
...
test("adding/removing components sync", async (t: any) => {
    var world = new World();
...
test("clearing pooled components", async (t: any) => {
    var world, entity;

    // Component with no constructor

    class BazComponent extends Component<any> {}
    ...
    t.is((entity.getComponent(BazComponent) as any).spam, "eggs", "property should be taken from addComponent args");
    ...
    t.is(
        (entity.getComponent(BazComponent) as any).spam,
        "",
    ...
    t.is(
        (entity.getComponent(PimComponent) as any).spam,
        "eggs",
...
test("removing components deferred", async (t: any) => {
    var world = new World();
...
test("remove entity", async (t: any) => {
    var world = new World();
...
test("get component development", async (t: any) => {
    var world = new World();
    ...
    t.is(entity.getComponent(FooComponent, true), component);
});

test("get component production", async (t: any) => {
    ...
    t.notThrows(() => ((component as any).variableFoo = 4));
    ...
    t.notThrows(() => ((removedComponent as any).variableFoo = 14));

    process.env.NODE_ENV = oldNodeEnv;
});

test("get removed component development", async (t: any) => {
    var world = new World();
    ...
    var entity = world.createEntity();
    entity.addComponent(FooComponent);
    var component = entity.getComponent(FooComponent);
    entity.removeComponent(FooComponent);

    t.is(entity.getRemovedComponent(FooComponent), component);
});

test("get removed component production", async (t: any) => {
    const oldNodeEnv = process.env.NODE_ENV;
    process.env.NODE_ENV = "production";
    var world = new World();
...
test("Delete entity from entitiesByNames", async (t: any) => {
    var world = new World();
    ...
    t.deepEqual({ entityA: entityA, entityB: entityB }, world.entityManager._entitiesByNames);
    ...
    t.deepEqual({ entityB: entityB }, world.entityManager._entitiesByNames);
    world.execute(0, 0); // Deferred remove happens
