...
import { createType, copyCopyable, cloneClonable, Types } from "../../src/Types";
import { Vector3 } from "../helpers/customtypes";

var CustomTypes = {
    Vector3: createType<Vector3>({
        name: "Vector3",
        default: new Vector3(),
        copy: copyCopyable,
        clone: cloneClonable,
...
test("default values", (t: any) => {
    const component: any = new TestComponent();
    ...
    t.not(component.jsonWithDefault, TestComponent.schema.jsonWithDefault.default);
    t.true(new Vector3(1, 2, 3).equals(component.vector3WithDefault));
});

test("copy component", (t: any) => {
    const srcComponent: any = new TestComponent();
    ...
    const destComponent: any = new TestComponent();
...
test("registering components before systems", (t: any) => {
    class ComponentA extends Component<any> {}
    class ComponentB extends Component<any> {}

    class SystemA extends System {
        static queries = { S: { components: [ComponentA, ComponentB] } };
    }
    ...
    t.is(error1.message, "Tried to create a query 'SystemA.S' with unregistered components: [ComponentA, ComponentB]");
    ...
    t.is(error2.message, "Tried to create a query 'SystemA.S' with unregistered components: [ComponentB]");
});
